...
![Tutorial Token Banxico](reference/figures/tokenbanxi2.gif)
...
Si utilizas el argumento `install = TRUE`, el token se guardará en tu
archivo `.Renviron` para que no tengas que escribirlo nunca más:
...
La estructura temporal de las tasas de interés es un pilar para la
valoración financiera y el modelado de riesgos. Al estar diseñado para
integrarse con el ecosistema moderno de R, puedes iterar fácilmente
sobre las funciones de `cetesR` para extraer las tasas actuales y
construir la curva de rendimiento libre de riesgo en instantes:
